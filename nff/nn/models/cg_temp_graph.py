...
class cg_temp_graph(nn.Module):
    ...
    def temp_scale(self, temp):
        return self.thermo_embed((temp.reshape(-1))) 
    ...
    def forward(self, batch):
        ...
        if self.temp_type == 'mult':
            results = self.atomwisereadout((r_sys * temp_learn.T) + (r_mol * temp_learn.T))
